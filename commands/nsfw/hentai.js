...
module.exports = class BoobsCommand extends Command {
    ...
    run(message) {
      ...
      uses the t!hentai command for the following reason:
      ...
      */
      ...
      /*
       if (message.channel.nsfw) {            
            fetch(`https://api.tenor.com/v1/random?key=${tenorAPI}&q=nsfw-hentai&limit=1`)
                .then(res => res.json())
      ...
      */
        ...
        if (message.channel.nsfw) {            
            try {
      const linkArray = fs
        .readFileSync('././resources/gifs/hentailinks.txt', 'utf8')
        .split('\n');
