...
var searchYoutube = async(query, message, voiceChannel) => {
    ...
    if (!videos) {
        message.reply(
            `:x: I had some trouble finding what you were looking for...\nPlease try again or be more specific.`
        );
        return;
    }
    if (videos.length < 5) {
            ...
            youtube
                ...
                .then(function(video) {
                    if (
                        ...
                        songEmbed.delete();
                        message.reply(
                            'Live streams are not permitted in this server!'
                        );
                        return;
                    }
                    ...
                    if (video.duration.hours !== 0 && !playVideosLongerThan1Hour) {
                        songEmbed.delete();
                        message.reply(
                            'Videos longer than 1 hour are not permitted in this server!'
                        );
                        return;
                    }
                    ...
                    if (message.guild.musicData.queue.length > maxQueueLength) {
                        songEmbed.delete();
                        message.reply(
                            'The queue hit its limit of ' + maxQueueLength + '...\nPlease wait a bit before attempting to add more songs.'
                        );
                        return;
                    }
                    message.guild.musicData.queue.push(
